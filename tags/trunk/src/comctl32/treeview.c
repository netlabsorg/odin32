/* $Id: treeview.c,v 1.28 2000-02-18 17:13:39 cbratschi Exp $ */
/* Treeview control
 *
...
/* CB: todo
 ...
 - TVS_INFOTIP, TVS_NOTOOLTIPS and tooltip support
...
TREEVIEW_SetItemHeight (HWND hwnd, WPARAM wParam)
{
  ...
  if (!(GetWindowLongA(hwnd, GWL_STYLE) & TVS_NONEVENHEIGHT))
    if (infoPtr->uItemHeight & 0x1) infoPtr->uItemHeight += 1;
...
//INT cChildren = TREEVIEW_HasChildren(hwnd,wineItem);
  ...
  if (item->firstChild && (item->state & TVIS_EXPANDED))
  {
    ...
    points[1].x = points[0].x = 28+20*item->iLevel-infoPtr->cx;
...
TREEVIEW_DrawItem(HWND hwnd,HDC hdc,TREEVIEW_ITEM *item,DWORD dwStyle,TREEVIEW_INFO *infoPtr)
{
  ...
  if (dwStyle & TVS_HASLINES)
  {
    ...
    if (dwStyle & TVS_LINESATROOT)
    {
      ...
      {
        points[1].x = 8+(20*item->iLevel)-infoPtr->cx;
        points[1].y = points[0].y = center;
        points[0].x = points[1].x + 10;
      }
  ...
  if (tvItem->mask & TVIF_TEXT)
  {
    if (unicode)
    {
      if (tvItem->pszText != LPSTR_TEXTCALLBACKW)
      {
        ...
        {
          wineItem->pszText= COMCTL32_ReAlloc(wineItem->pszText,len*sizeof(WCHAR));
...
static BOOL TREEVIEW_CalcItems(HWND hwnd,HDC hdc,TREEVIEW_INFO *infoPtr)
{
  ...
  if (!(dwStyle & TVS_NONEVENHEIGHT))
    if (itemHeight & 0x1) itemHeight += 1; //important for PS_DOT pen!
  ...
  while (iItem)
  {
    ...
    } else
    {
      ...
      xOffset = (x-infoPtr->cx)-item->rect.left;
      yOffset = (y-infoPtr->cy)-item->rect.top;
    ...
    infoPtr->uTotalWidth = MAX(infoPtr->uTotalWidth,item->rect.right+infoPtr->cx);
  ...
  //check cx and cy
  maxCY = infoPtr->uTotalHeight-infoPtr->uVisibleHeight;
  if (maxCY < 0) maxCY = 0;
  if (infoPtr->cy > maxCY)
  {
    yDiff = maxCY-infoPtr->cy;
    infoPtr->cy = maxCY;
  }

  maxCX = infoPtr->uTotalWidth-infoPtr->uVisibleWidth;
  if (maxCX < 0) maxCX = 0;
  if (infoPtr->cx > maxCX)
  {
    xDiff = maxCX-infoPtr->cx;
    infoPtr->cx = maxCX;
  }
  ...
  if (tvItem->mask & TVIF_TEXT)
  {
    ...
    if (unicode)
    {
      if (tvItem->pszText != LPSTR_TEXTCALLBACKW)
      {
        ...
        len = lstrlenW(tvItem->pszText)+1;
        wineItem->pszText = COMCTL32_Alloc(len*sizeof(WCHAR));
...
#if 1 //CB: synchron update conflicts with Odin32 implementation -> check and fix
      TREEVIEW_QueueRefresh(hwnd);
#else
      TREEVIEW_Refresh(hwnd); //because size redraw isn't set
#endif
  }
...
TREEVIEW_Create (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  if ((TREEVIEW_INFO*)GetWindowLongA(hwnd,0) != infoPtr) return 0;

  hdc = GetDC(hwnd);

  /* set default settings */
  infoPtr->uInternalStatus = 0;
  if (IsWindowUnicode(hwnd)) infoPtr->uInternalStatus |= TV_UNICODE;
  infoPtr->uNumItems = 0;
  ...
  if (!(dwStyle & TVS_NOTOOLTIPS))
  {
    ...
    if (infoPtr->hwndToolTip)
    {
      ...
      SendMessageA (GetParent(hwnd),WM_NOTIFY,(WPARAM)GetWindowLongA(hwnd,GWL_ID),(LPARAM)&nmttc);
    }
...

{
  ...
  BOOL unicode = IsWindowUnicode(parent),rc;
...
//                code,action,(INT)oldItem,(INT)newItem);
  ...
  if (unicode)
    rc = (BOOL)SendMessageW(parent,WM_NOTIFY,(WPARAM)GetWindowLongW(hwnd,GWL_ID),(LPARAM)&nmhdr);
  else
    rc = (BOOL)SendMessageA(parent,WM_NOTIFY,(WPARAM)GetWindowLongA(hwnd,GWL_ID),(LPARAM)&nmhdr);
...
TREEVIEW_SendDispInfoNotify (HWND hwnd, TREEVIEW_ITEM *wineItem, UINT code, UINT what)
{
  ...
  WCHAR *buf;
  HWND parent = GetParent(hwnd);
  BOOL unicode = IsWindowUnicode(parent);
  ...
  tvdi.item.pszText     = COMCTL32_Alloc(128*(unicode? sizeof(WCHAR):sizeof(char)));
  tvdi.item.cchTextMax = 128;
  buf = tvdi.item.pszText;

  if (unicode)
    retval = (BOOL)SendMessageW(parent,WM_NOTIFY,(WPARAM)tvdi.hdr.idFrom,(LPARAM)&tvdi);
  else
  ...
  if (what & TVIF_TEXT)
  {
    ...
    } else
    {
      if (buf == tvdi.item.pszText)
      {
        COMCTL32_Free(wineItem->pszText);
        wineItem->cchTextMax = 128;
        wineItem->pszText = COMCTL32_Alloc(128*sizeof(WCHAR));
...
TREEVIEW_EndEditLabelNow (HWND hwnd, BOOL bCancel)
{
  ...
  if (!bCancel)
  {
    textW = COMCTL32_Alloc(1024*sizeof(WCHAR));
    ...
    if (unicode) tvdi.item.pszText = textW; else
    {
      INT len = iLength+1;

      textA = COMCTL32_Alloc(len);
  ...
  if (unicode)
    bCommit=(BOOL)SendMessageW(parent,WM_NOTIFY,(WPARAM)tvdi.hdr.idFrom,(LPARAM)&tvdi);
  else
...
TREEVIEW_LButtonDoubleClick (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  if (!TREEVIEW_SendSimpleNotify (hwnd, NM_DBLCLK))
...
static LRESULT TREEVIEW_MouseMove(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  if (!TREEVIEW_HitTest(hwnd,&ht)) return 0;
...
{
  TREEVIEW_SendSimpleNotify(hwnd,NM_RDBLCLK);
...
TREEVIEW_DoSelectItem (HWND hwnd, INT action, HTREEITEM newSelect, INT cause)
{
  ...
  switch (action)
  {
    ...
    case TVGN_FIRSTVISIBLE:
    {
      ...
      {
        scrollY = MIN(wineItem->rect.top,infoPtr->uTotalHeight-infoPtr->uVisibleHeight);
...
TREEVIEW_SetFont (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  if (lParam)
  {
    ...
    TREEVIEW_CalcItems(hwnd,0,infoPtr);
    TREEVIEW_Refresh(hwnd);
  } else infoPtr->uInternalStatus |= TV_CALCALL;
...
TREEVIEW_VScroll (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD (wParam))
  {
    ...
    case SB_THUMBTRACK:
      ...
      if (infoPtr->cy < 0) infoPtr->cy = 0;
      if (infoPtr->cy > maxHeight) infoPtr->cy = maxHeight;
      break;
  ...
  if (lastPos != infoPtr->cy)
  {
    if (!TREEVIEW_UnqueueRefresh(hwnd,TRUE,TRUE))
      TREEVIEW_CalcItems(hwnd,0,infoPtr);
    ScrollWindowEx(hwnd,0,lastPos-infoPtr->cy,NULL,NULL,0,NULL,SW_INVALIDATE | SW_SCROLLCHILDREN | (infoPtr->uScrollTime << 16));
...
TREEVIEW_HScroll (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD (wParam))
  {
    ...
    case SB_THUMBTRACK:
      ...
      if (infoPtr->cx < 0) infoPtr->cx = 0;
      if (infoPtr->cx > maxWidth) infoPtr->cx = maxWidth;
      break;
  ...
  if (lastPos != infoPtr->cx)
  {
    if (!TREEVIEW_UnqueueRefresh(hwnd,TRUE,TRUE))
      TREEVIEW_CalcItems(hwnd,0,infoPtr);
    ScrollWindowEx(hwnd,lastPos-infoPtr->cx,0,NULL,NULL,0,NULL,SW_INVALIDATE | SW_SCROLLCHILDREN | (infoPtr->uScrollTime << 16));
...
TREEVIEW_KeyDown (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (wParam)
  {
    ...
    case VK_RETURN:
      TREEVIEW_SendSimpleNotify(hwnd,NM_RETURN);
      break;
...
static TREEVIEW_ITEM* TREEVIEW_SearchChildren(HWND hwnd,TREEVIEW_INFO *infoPtr,TREEVIEW_ITEM *item,TREEVIEW_ITEM *stop,LPWSTR text,INT textlen)
{
  TREEVIEW_ITEM *child,*child2;

  if (!(item->firstChild && (item->state & TVIS_EXPANDED))) return NULL;

  child = &infoPtr->items[(INT)item->firstChild];
  while (child)
  {
    if (child == stop) return NULL;
    if (TREEVIEW_Compare(hwnd,child,text,textlen)) return child;
    child2 = TREEVIEW_SearchChildren(hwnd,infoPtr,child,stop,text,textlen);
    if (child2) return child2;
    if (child->sibling) child = &infoPtr->items[(INT)child->sibling];
    else return NULL;
  }
...
static TREEVIEW_ITEM* TREEVIEW_Search(HWND hwnd,TREEVIEW_INFO *infoPtr,INT iItem,LPWSTR text,INT textlen)
{
  ...
  if (TREEVIEW_Compare(hwnd,item,text,textlen)) found = TRUE; else
  {
    //search start to end
    while (item->sibling || item->parent)
    {
      TREEVIEW_ITEM *child = TREEVIEW_SearchChildren(hwnd,infoPtr,item,NULL,text,textlen);
      ...
      if (item && TREEVIEW_Compare(hwnd,item,text,textlen))
      {
        found = TRUE;
        break;
      }
    ...
    if (!found)
    {
      ...
      while (item != start)
      {
        ...
        if (TREEVIEW_Compare(hwnd,item,text,textlen))
        {
          found = TRUE;
          break;
        }
        child = TREEVIEW_SearchChildren(hwnd,infoPtr,item,start,text,textlen);
        ...
        if (item->sibling) item = &infoPtr->items[(INT)item->sibling];
        else break;
      }
    }
  }
...
static VOID TREEVIEW_ISearch(HWND hwnd,CHAR ch)
{
  ...
  newString = COMCTL32_Alloc((len+1)*sizeof(WCHAR));
  ...
  if (!item && infoPtr->pszISearch)
  {
    TREEVIEW_ITEM *next;
//CB: todo: must press key twice to go to the next item
    item = &infoPtr->items[iItem];
    next = TREEVIEW_GetNextListItem(hwnd,infoPtr,item);
    if (!next) next = &infoPtr->items[(INT)infoPtr->TopRootItem];
    if (next && (next != item))
    {
      next = TREEVIEW_Search(hwnd,infoPtr,(INT)next->hItem,infoPtr->pszISearch,infoPtr->uISearchLen);
      if (next)
...
{
  TREEVIEW_SendSimpleNotify(hwnd,NM_SETCURSOR); //CB: todo: result

  return DefWindowProcA(hwnd,WM_SETCURSOR,wParam,lParam);
}

static LRESULT TREEVIEW_GetUnicodeFormat(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  TREEVIEW_INFO *infoPtr = TREEVIEW_GetInfoPtr(hwnd);

  return infoPtr->uInternalStatus & TV_UNICODE;
}

static LRESULT TREEVIEW_SetUnicodeFormat(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  TREEVIEW_INFO *infoPtr = TREEVIEW_GetInfoPtr(hwnd);
  BOOL oldFormat = infoPtr->uInternalStatus & TV_UNICODE;

  if (wParam)
    infoPtr->uInternalStatus |= TV_UNICODE;
  else
    infoPtr->uInternalStatus &= ~TV_UNICODE;
   ...
   if (!TREEVIEW_GetInfoPtr(hwnd))
    ...
    switch (uMsg)
    {
        ...
        case TVM_GETUNICODEFORMAT:
                return TREEVIEW_GetUnicodeFormat(hwnd,wParam,lParam);

        case TVM_SETUNICODEFORMAT:
                return TREEVIEW_SetUnicodeFormat(hwnd,wParam,lParam);

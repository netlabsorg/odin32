...
#ifdef __WIN32OS2__
                //YD: 2000-12-01: extra check added for loops in menus
                              ...
                              HWND hwnd, const RECT *lprect )
{
     ...
     * clear that menu loop is not over yet. */
        ...
        for (;;)
        {
            ...
            {
#ifdef __WIN32OS2__
                //We need to get & remove the message immediately, otherwise
                //we'll run into an Odin msg handling limitation (see oslibmsg.cpp todo's)
            ...
            if (!(wFlags & TPM_RIGHTBUTTON)) {
              ...
              UINT id = 0;
                       ...
                       if( IS_SYSTEM_MENU(menu) )
                          ...
                          item = MENU_FindItemByCoords( menu, mt.pt, &id );
...
void MENU_TrackKbdMenuBar( HWND hwnd, UINT wParam, INT vkey)
{
...
#ifdef __WIN32OS2__
        if ((GetWindowLongA( hwnd, GWL_EXSTYLE ) & WS_EX_TOOLWINDOW)) return;
#endif
    ...
    if( vkey && vkey != VK_SPACE )
    {
        ...
        {
#ifndef __WIN32OS2__
            //Don't beep when unable to find menu item when alt key pressed
...
BOOL WINAPI SetSystemMenu( HWND hwnd, HMENU hMenu )
{
    ...
    hSysMenu = getSysMenu(hwnd);
    if(hSysMenu) DestroyMenu(hSysMenu);

    setSysMenu(hwnd, MENU_GetSysMenu( hwnd, hMenu ));
    return TRUE;
#else
    WND *wndPtr = WIN_FindWndPtr(hwnd);
...
HMENU WINAPI GetMenu( HWND hWnd )
{
    ...
    dprintf(("WARNING: Invalid menu %x", retvalue));
    return 0;
#else
...
#ifdef __WIN32OS2__
                    lstrtrunc((LPSTR)lpmii->dwTypeData, lpmii->cch );
#else
                                      ...
                                      BYTE fVirt,WORD key,WORD cmd)
{
    ...
    else if( key >= 1 && key <= 26 && !( fVirt & FVIRTKEY )) // Ctrl-A to Ctrl-Z
    {
        int ctrlCh;

        if( msg->message == WM_CHAR )
            ctrlCh = toupper( msg->wParam ) - 'A' + 1;
        else
            ctrlCh = msg->wParam - VK_A + 1;
        ...
        {
            INT     iSysStat = -1,iStat = -1,mesg=0;
            HMENU   hSysMenu, hMenu, hSubMenu;
            UINT    nPos;
            ...
            if (msg->message == WM_KEYUP || msg->message == WM_SYSKEYUP) {
                ...
                {
                    hMenu = (GetWindowLongA(hWnd, GWL_STYLE) & WS_CHILD) ? NULL : GetMenu(hWnd);
                    hSysMenu = getSysMenu(hWnd);
                    ...
                    if(MENU_FindItem(&hSubMenu, &nPos, MF_BYCOMMAND))
                    {
                        ...
                        if(hSubMenu != hSysMenu)
                        {
                            ...
                            dprintf(("hSysMenu = %04x, hSubMenu = %04x, nPos = %d\n", hSysMenu, hSubMenu, nPos));
                            SendMessageA(hWnd, WM_INITMENUPOPUP, (WPARAM)hSubMenu, MAKELPARAM(nPos, TRUE));
                        }
                        iSysStat = GetMenuState(GetSubMenu(hSysMenu, 0), cmd, MF_BYCOMMAND);
                    }
                    else
                    {
                        hSubMenu = hMenu;
                        ...
                        if(MENU_FindItem(&hSubMenu, &nPos, MF_BYCOMMAND))
                        {
                            ...
                            if(hSubMenu != hMenu)
                            {
                                ...
                                dprintf(("hMenu = %04x, hSubMenu = %04x, nPos = %d\n", hMenu, hSubMenu, nPos));
                                SendMessageA(hWnd, WM_INITMENUPOPUP, (WPARAM)hSubMenu, MAKELPARAM(nPos, FALSE));
                            }
                            iStat = GetMenuState(hMenu, cmd, MF_BYCOMMAND);
                        }
                    }
                ...
                if ( mesg==WM_COMMAND )
                    SendMessageA(hWnd, mesg, MAKEWPARAM( cmd, 1 ), NULL);
                else if( mesg==WM_SYSCOMMAND )
                    SendMessageA( hWnd, mesg, cmd, MAKELPARAM( 0, 1 ));
                else
                {

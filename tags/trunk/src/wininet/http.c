 ...
 *
 */
HINTERNET HTTP_HttpOpenRequestA(HINTERNET hHttpSession,
        LPCSTR lpszVerb, LPCSTR lpszObjectName, LPCSTR lpszVersion,
        LPCSTR lpszReferrer , LPCSTR *lpszAcceptTypes,
        DWORD dwFlags, DWORD dwContext)
{
    ...
    if (NULL == lpszVerb)
        lpwhr->lpszVerb = strdup("GET");
    else if (strlen(lpszVerb))
        lpwhr->lpszVerb = strdup(lpszVerb);
    ...
    if (NULL != lpszReferrer)
    {
        ...
        if (strlen(UrlComponents.lpszHostName))
            lpwhr->lpszHostName = strdup(UrlComponents.lpszHostName);
    } else {
        lpwhr->lpszHostName = strdup(lpwhs->lpszServerName);
    }
        ...
        if (index == HTTP_QUERY_RAW_HEADERS_CRLF || index == HTTP_QUERY_RAW_HEADERS)
        {
           ...
           for (i = 0; i <= HTTP_QUERY_MAX; i++)
           {
               if ((~lpwhr->StdHeaders[i].wFlags & HDR_ISREQUEST) &&
                                           ...
                                           lpwhr->StdHeaders[i].lpszValue)
               {
                   cnt += sprintf((LPBYTE)lpBuffer + cnt, "%s: %s%s", lpwhr->StdHeaders[i].lpszField, lpwhr->StdHeaders[i].lpszValue,
            ...
            for (i = 0; i < lpwhr->nCustHeaders; i++)
            {
                if ((~lpwhr->pCustHeaders[i].wFlags & HDR_ISREQUEST) &&
                                                ...
                                                lpwhr->pCustHeaders[i].lpszValue)
                {
                   cnt += sprintf((LPBYTE)lpBuffer + cnt, "%s: %s%s",
                    lpwhr->pCustHeaders[i].lpszField, lpwhr->pCustHeaders[i].lpszValue,
                                        index == HTTP_QUERY_RAW_HEADERS_CRLF ? "\r\n" : "\0");
                }
            }

            strcpy((LPBYTE)lpBuffer + cnt, index == HTTP_QUERY_RAW_HEADERS_CRLF ? "\r\n" : "");
        ...
        DWORD dwHeaderLength, LPVOID lpOptional ,DWORD dwOptionalLength)
{
    ...
    if (hIC->hdr.dwFlags & INTERNET_FLAG_ASYNC)
    {
        ...
        workRequest.LPSZHEADER = (DWORD)strdup(lpszHeaders);
 ...
 *
 */
BOOL HTTP_HttpSendRequestA(HINTERNET hHttpRequest, LPCSTR lpszHeaders,
        DWORD dwHeaderLength, LPVOID lpOptional ,DWORD dwOptionalLength)
{
    INT cnt;
    INT i;
        ...
        LPCSTR lpszPassword, DWORD dwFlags, DWORD dwContext)
{
    ...
    if (NULL != lpszServerName)
        lpwhs->lpszServerName = strdup(lpszServerName);
    if (NULL != lpszUserName)
        lpwhs->lpszUserName = strdup(lpszUserName);
...
BOOL HTTP_ProcessHeader(LPWININETHTTPREQA lpwhr, LPCSTR field, LPCSTR value, DWORD dwModifier)
{
    ...
    if (!lphttpHdr->lpszValue && (dwModifier & (HTTP_ADDHDR_FLAG_ADD|HTTP_ADDHDR_FLAG_ADD_IF_NEW)))
    {
        ...
        {
            lphttpHdr->lpszField = strdup(field);
...
INT HTTP_InsertCustomHeader(LPWININETHTTPREQA lpwhr, LPHTTPHEADERA lpHdr)
{
    ...
    if (NULL != lph)
    {
        ...
        lpwhr->pCustHeaders[count-1].lpszField = strdup(lpHdr->lpszField);
        lpwhr->pCustHeaders[count-1].lpszValue = strdup(lpHdr->lpszValue);

/* $Id: winimagelx_new.cpp,v 1.1 2001-05-30 18:32:15 phaller Exp $ */
               ...
               : Win32ImageBase(hInstance), header(0)
{
 APIRET rc;
  ...
  // @@@PH look for way to reduce memory consumption
  // Note: never seen a DLL export > 8192 or something
  pLookupOrdinal = new CIndexLookupLimit(0, 8192);
...
ULONG Win32LxImage::getApi(char *name)
{
  ...
  apiaddr = (ULONG)pLookupName->getElement(name);
  if (apiaddr)
      return apiaddr;
  ...
  // add to cache
  pLookupName->addElement(name, (void*)apiaddr);
...
ULONG Win32LxImage::getApi(int ordinal)
{
    ...
    // check lookup cache first
    apiaddr = (ULONG)pLookupOrdinal->getElement(ordinal);
    if (apiaddr)
        return apiaddr;
    ...
    // add to cache
    pLookupOrdinal->addElement(ordinal, (void*)apiaddr);

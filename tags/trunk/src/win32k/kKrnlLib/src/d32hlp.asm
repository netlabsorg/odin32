; $Id: d32hlp.asm,v 1.1 2002-03-10 02:45:54 bird Exp $
;
...
;
;D32RET D32HLPCALL D32Hlp_RAS(ULONG      major,       /* eax */
;                             ULONG      cbBuffer,    /* edx */
;                             ULONG      minor,       /* ecx */
;                             ULONG      pf16Buffer); /* esp + 10h */
D32Hlp_RAS proc near
    push    esi
    push    ebx
    push    ds
    ...
    mov     ebx, edx
    mov     dl, DevHlp_RAS
    jmp     far ptr CODE16:Thunk16_RAS
Thunk32_RAS::
    jc      Finished
    xor     eax, eax
...
Thunk16_RAS:
    cmp     es:_Device_Help, 0          ; during init this may be NULL, especially when debuging in ring-3.
    je      Thunk16_RAS_skip
    call    es:[_Device_Help]
Thunk16_RAS_skip:
    jmp far ptr FLAT:Thunk32_RAS

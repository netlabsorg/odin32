...
{
  XVisualInfo *vinfo, **vlist, _template;
...
#if !defined(_WIN32) && !defined(__WIN32OS2__)
    _template.screen = __glutScreen;
    vinfo = XGetVisualInfo(__glutDisplay, VisualScreenMask, &_template, &n);
#else
    vinfo = XGetVisualInfo(__glutDisplay, 0, &_template, &n);
#endif
    if (vinfo == NULL) {
      *nitems_return = 0;
      return NULL;
...
#ifdef _WIN32 || defined(__WIN32OS2__)
          printf("Visual = 0x%x\n", 0);
#else

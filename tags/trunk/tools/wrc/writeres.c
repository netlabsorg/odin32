...
#define MASM 1
    ...
    #define LOCAL_PREFIX ""
    #define LONGFRMT        "0%08lxh"
    #define SHORTFRMT       "0%04xh"
    #define BYTEFRMT        "0%02xh"
    ...
    #define LOCAL_PREFIX     "."
    #define LONGFRMT        "0x%08lx"
    #define SHORTFRMT       "0x%04x"
    #define BYTEFRMT        "0x%02x"
    #define HEXBIT31        "0x80000000"
    #define COMMENT_LINE
    #define OR              "|"
#endif
...
char s_file_head_str[] =
    COMMENT_LINE"/* This file is generated with wrc version " WRC_FULLVERSION ". Do not edit! */\n"
    COMMENT_LINE"/* Source : %s */\n"
    COMMENT_LINE"/* Cmdline: %s */\n"
    COMMENT_LINE"/* Date   : %s */\n"
                "\n"
    #ifdef MASM
                "\t.386p\n"
...
void write_ne_segment(FILE *fp, resource_t *top)
{
    ...
    for(i = 0; i < rccount; i++)
    {
        ...
        /* TypeId */
        if(rcp->type.type == name_ord)
            fprintf(fp, "\t"DIRECTIVE_WORD"\t"SHORTFRMT"\n", rcp->type.name.i_name | 0x8000);
        else
            fprintf(fp, "\t"DIRECTIVE_WORD"\t%s_%s_typename - %s%s\n",
                prefix,
 ...
 */
            /* Offset */
            fprintf(fp, "\t"DIRECTIVE_WORD"\t(%s%s_data - %s%s) >> %d\n",
                prefix,
            ...
            if(rcp->rscarray[j]->name->type == name_ord)
                ...
                fprintf(fp, "\t"DIRECTIVE_WORD"\t%s%s_name - %s%s\n",
                prefix,
                rcp->rscarray[j]->c_name,
                prefix,
                _NEResTab);
            /* Handle and Usage */
            fprintf(fp, "\t"DIRECTIVE_WORD"\t0, 0\n");
        }
    }
    /* EndTypes */
    fprintf(fp, "\t"DIRECTIVE_WORD"\t0\n");
}
...
void write_s_file(char *outname, resource_t *top)
{
    ...
    if(indirect)
    {
        ...
        for(rsc = top; rsc; rsc = rsc->next)
        {
            ...
            fprintf(fo, "\t"DIRECTIVE_LONG"\t%d, %s%s%s%s, %d, %s%s%s%s, %s%s_data, %d\n",
        ...
        fprintf(fo, COMMENT_LINE"/* Resource indirection table */\n\n");
        fprintf(fo, "\t"DIRECTIVE_ALIGN"\t4\n");
        fprintf(fo, "%s%s:\n", prefix, _ResTable);
        fprintf(fo, "\t"DIRECTIVE_GLOBAL"\t%s%s\n", prefix, _ResTable);
        for(rsc = top; rsc; rsc = rsc->next)

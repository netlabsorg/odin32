...
BOOL WIN32API hookInit(ULONG hab)
{
    ...
    {//query odin client window class name
        strcpy(szWindowClass, QueryCustomStdClassName());
    }
    return WinSetHook(hab, HMQ_CURRENT, // queue hook
...
{
    WinReleaseHook(hab, HMQ_CURRENT,
                   HK_PREACCEL,     // pre-accelerator table hook (undocumented)
                   (PFN)hookPreAccelHook,
                   NULL);
    return TRUE;
...
BOOL i_isWin32Window(HWND hwnd)
{
  ...
  {
    if (strcmp(szClass, szWindowClass) == 0)
    {

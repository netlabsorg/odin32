/* $Id: bitmap.cpp,v 1.1 1999-10-06 18:50:50 bird Exp $ */
...
BOOL ShowBitmap(LXHeaderSuper &OS2Exe, int id, WINBITMAPINFOHEADER *bhdr, int size)
{
    ...
    pConvertedBitmap = ConvertBitmap(bhdr, size, (PULONG)SSToDS(&ulSize));
    if (pConvertedBitmap != NULL)
    {
        rc = OS2Exe.StoreResource(id, RT_BITMAP, ulSize, (char *)pConvertedBitmap);

        #ifdef DUMP_BMPS_TO_FILES
            /* debug option to dump bitmaps to file(s). */
            char szFilename[CCHMAXPATH];
            sprintf(SSToDS(szFilename), "%d.bmp", id);
            ...
            sprintf(SSToDS(szFilename), "w%d.bmp", id);
    ...
    return rc;
#else
    return OS2Exe.StoreResource(id, RT_BITMAP, size, (char*) bhdr);
#endif
}

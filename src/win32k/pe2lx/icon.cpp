/* $Id: icon.cpp,v 1.1 1999-09-06 02:20:04 bird Exp $ */
...
id(0), next(NULL), iconhdr(NULL), iconsize(0), prevoffset(0)
{
    ...
    vltassert((ULONG)bmpHdr > MINPTR && (ULONG)bmpHdr+size < MAXPTR);
  ...
  if(bmpHdr->biSizeImage < colorsize) {
    ...
    if (iconhdr == NULL)
    {
        if (icons == this)
            ...
            icon->next = NULL;
        vltassert(iconhdr != NULL);
    }
    memset((void*)iconhdr, 0xaa, iconsize);     //@KSO: this makes comparing converteded files easier

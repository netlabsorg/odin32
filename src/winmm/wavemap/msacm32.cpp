...
static	DWORD	wodOpen(LPDWORD lpdwUser, LPWAVEOPENDESC lpDesc, DWORD dwFlags)
{
    ...
    UINT		i;
    WAVEMAPDATA*	wom = (WAVEMAPDATA*)HeapAlloc(GetProcessHeap(), 0, sizeof(WAVEMAPDATA));
    WAVEFORMATEX	wfx;
	...
	*(VOID **)wom->acmStreamOpen            = (void*)GetProcAddress(hModule, "acmStreamOpen");
	*(VOID **)wom->acmStreamClose           = (void*)GetProcAddress(hModule, "acmStreamClose");
	*(VOID **)wom->acmStreamSize            = (void*)GetProcAddress(hModule, "acmStreamSize");
	*(VOID **)wom->acmStreamConvert         = (void*)GetProcAddress(hModule, "acmStreamConvert");
	*(VOID **)wom->acmStreamPrepareHeader   = (void*)GetProcAddress(hModule, "acmStreamPrepareHeader");
	*(VOID **)wom->acmStreamUnprepareHeader = (void*)GetProcAddress(hModule, "acmStreamUnprepareHeader");
    } while (0);
...
static	DWORD	wodPrepare(WAVEMAPDATA* wom, LPWAVEHDR lpWaveHdrSrc, DWORD dwParam2)
{
    ...
    ash = (_ACMSTREAMHEADER*)HeapAlloc(GetProcessHeap(), 0, sizeof(ACMSTREAMHEADER) + sizeof(WAVEHDR) + size);
    if (ash == NULL)
	return MMSYSERR_NOMEM;
    ...
    ash->pbSrc = (BYTE*)lpWaveHdrSrc->lpData;
    ash->cbSrcLength = lpWaveHdrSrc->dwBufferLength;
    /* ash->cbSrcLengthUsed */
    ash->dwSrcUser = lpWaveHdrSrc->dwUser; /* FIXME ? */
    ash->pbDst = (BYTE*)((LPSTR)ash + sizeof(ACMSTREAMHEADER) + sizeof(WAVEHDR));
    ...
    lpWaveHdrDst = (LPWAVEHDR)((LPSTR)ash + sizeof(ACMSTREAMHEADER));
    lpWaveHdrDst->lpData = (CHAR*)ash->pbDst;
...
static	DWORD	widOpen(LPDWORD lpdwUser, LPWAVEOPENDESC lpDesc, DWORD dwFlags)
{
    ...
    UINT		i;
    WAVEMAPDATA*	wim = (WAVEMAPDATA*)HeapAlloc(GetProcessHeap(), 0, sizeof(WAVEMAPDATA));

/* $Id: dlgconvert.cpp,v 1.3 1999-06-10 16:50:38 phaller Exp $ */
...
//BS_TEXT, BS_BITMAP, BS_ICON, BS_MINIICON can only be used with BS_PUSHBUTTON
  	...
  	case WIN_EDIT:
		*os2class = (int)WC_ENTRYFIELD & 0xFFFF;
		if(style & WINES_LEFT)
			os2style |= ES_LEFT;		
		if(style & WINES_CENTER)
			os2style |= ES_CENTER;
		if(style & WINES_RIGHT)
			os2style |= ES_RIGHT;
#if 0
		if(style & WINES_MULTILINE)
			os2style |= ;
		if(style & WINES_UPPERCASE)
			os2style |= ;
		if(style & WINES_LOWERCASE)
			os2style |= ;
#endif
		if(style & WINES_PASSWORD)
			os2style |= ES_UNREADABLE;
#if 0
		if(style & WINES_AUTOVSCROLL)
			os2style |= ;
		if(style & WINES_AUTOHSCROLL)
			os2style |= ES_AUTOSCROLL;
...
//			os2style |= ;
  	...
  	case WIN_SCROLLBAR:
		*os2class = (int)WC_SCROLLBAR & 0xFFFF;
		if(style & WINSBS_HORZ)
			os2style |= SBS_HORZ;
		else
		if(style & WINSBS_VERT)
			os2style |= SBS_VERT;
#if 0
		...
		if(style & WINSBS_BOTTOMALIGN)
			os2style |= ;
		if(style & WINSBS_RIGHTALIGN)
			os2style |= ;
		if(style & WINSBS_SIZEBOXTOPLEFTALIGN)
			os2style |= ;
		if(style & WINSBS_SIZEBOXBOTTOMRIGHTALIGN)
			os2style |= ;
#endif
		if(style & WINSBS_SIZEBOX)
			os2style |= SBS_AUTOSIZE;	//TODO: Correct?
		break;
  	case WIN_COMBOBOX:
		*os2class = (int)WC_COMBOBOX & 0xFFFF;  
		if(style & WINCBS_SIMPLE)
			os2style |= CBS_SIMPLE;
		else
		if(style & WINCBS_DROPDOWN)
			os2style |= CBS_DROPDOWN;
		else
		if(style & WINCBS_DROPDOWNLIST)
			os2style |= CBS_DROPDOWNLIST;
#if 0

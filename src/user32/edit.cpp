...
#define EDIT_SEND_CTLCOLOR(hwnd,hdc) \
        (SendMessageA(GetParent(hwnd), WM_CTLCOLOREDIT, \
                        (WPARAM)(hdc), (LPARAM)hwnd))
#define EDIT_NOTIFY_PARENT(hwnd, wNotifyCode, str) \
        do { \
...
static BOOL EDIT_MakeFit(HWND hwnd, EDITSTATE *es, INT size)
{
        ...
        if (size > es->buffer_limit) {
                EDIT_NOTIFY_PARENT(hwnd, EN_MAXTEXT, "EN_MAXTEXT");
                return FALSE;
        }
        ...
        if (es->buffer_size < size) {
                ...
                EDIT_NOTIFY_PARENT(hwnd, EN_ERRSPACE, "EN_ERRSPACE");
                return FALSE;
        } else {
                EDIT_LockBuffer(hwnd, es);
...
static LRESULT EDIT_EM_GetThumb(HWND hwnd, EDITSTATE *es)
{
        return MAKELONG(EDIT_WM_VScroll(hwnd, es, EM_GETTHUMB, 0, 0),
                EDIT_WM_HScroll(hwnd, es, EM_GETTHUMB, 0, 0));
}
...
static BOOL EDIT_EM_LineScroll(HWND hwnd, EDITSTATE *es, INT dx, INT dy)
{
        ...
        if (dx && !(es->flags & EF_HSCROLL_TRACK))
                EDIT_NOTIFY_PARENT(hwnd, EN_HSCROLL, "EN_HSCROLL");
        if (dy && !(es->flags & EF_VSCROLL_TRACK))
                EDIT_NOTIFY_PARENT(hwnd, EN_VSCROLL, "EN_VSCROLL");
        return TRUE;
}
 ...
 *
 */
static BOOL EDIT_CheckCombo(HWND hwnd, UINT msg, INT key, DWORD key_data)
{
        HWND hLBox;
...
static LRESULT EDIT_WM_KeyDown(HWND hwnd, EDITSTATE *es, INT key, DWORD key_data)
{
        ...
        case VK_F4:
        case VK_UP:
                if (EDIT_CheckCombo(hwnd, WM_KEYDOWN, key, key_data))
                        break;
                if (key == VK_F4)
                        break;
                /* fall through */
...
static LRESULT EDIT_WM_KillFocus(HWND hwnd, EDITSTATE *es, HWND window_getting_focus)
{
        ...
        EDIT_NOTIFY_PARENT(hwnd, EN_KILLFOCUS, "EN_KILLFOCUS");
        return 0;
}
...
static void EDIT_WM_Paint(HWND hwnd, EDITSTATE *es,WPARAM wParam)
{
        ...
        if (es->flags & EF_UPDATE) {
                es->flags &= ~EF_UPDATE;
                EDIT_NOTIFY_PARENT(hwnd, EN_CHANGE, "EN_CHANGE");
        }
}

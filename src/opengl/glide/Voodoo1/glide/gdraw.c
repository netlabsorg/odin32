...
**
** $Header: /home/ktk/tmp/odin/2007/netlabs.cvs/odin32/src/opengl/glide/Voodoo1/glide/Attic/gdraw.c,v 1.1 2000-02-18 10:43:06 sandervl Exp $
** $Log: gdraw.c,v $
** Revision 1.1  2000-02-18 10:43:06  sandervl
...
#define FP_TRUNC_BIAS(__fpVal, __fpBias) \
  ...
  while (i) {
    if (i & 1) {        /* packer bug check */
      ...
      GR_SETF( *dlp->addr, pfp[i>>2] /* FARRAY(p,i) */ );
    dlp++;
    i = dlp->i;
  }
   ...
   if (j >= i ) {                       /* if (adx > ady) */
     ...
     while (i) {
       ...
       if (i & 1) {     /* packer bug check */
         ...
         dp = pfa[i>>2] ; // FARRAY(a,i);
         GR_SETF( fp[0], dp );
         dp = pfb[i>>2] - dp ; // FARRAY(b,i) - dp;

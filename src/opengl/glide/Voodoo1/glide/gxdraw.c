...
**
** $Header: /home/ktk/tmp/odin/2007/netlabs.cvs/odin32/src/opengl/glide/Voodoo1/glide/Attic/gxdraw.c,v 1.1 2000-02-18 10:43:13 sandervl Exp $
** $Log: gxdraw.c,v $
** Revision 1.1  2000-02-18 10:43:13  sandervl
...
//#define FARRAY(p,i) (*(float *)((i)+(int)(p)))
  ...
  while (i) {
    ...
    if (i & 1) 
      ...
      dpBC = fb[i>>2] ; // FARRAY(fb,i);
      dpdx = fa[i>>2] ; // FARRAY(fa,i);
      FSET_GW_ENTRY( fifoPtr, 0, dpdx );
      dpAB = (float)((double)dpdx - (double)dpBC);
      dpBC = (float)((double)dpBC - (double)fc[i>>2]) ; // FARRAY(fc,i);
      dpdx = (float)((double)(dpAB * dyBC) - (double)(dpBC * dyAB));
      FSET_GW_ENTRY( fifoPtr, 1, dpdx );
      dpdy = (float)((double)(dpBC * dxAB) - (double)(dpAB * dxBC));
      dlp++;
      i = dlp->i;
  ...
  */
  while (i) {
    ...
    if (i & 1) {                   /* packer bug check */
      ...
      GR_SETF( fp[0], fa[i>>2] /* FARRAY(fa,i) */ );
      dlp++;
      i = dlp->i;
    }
  }

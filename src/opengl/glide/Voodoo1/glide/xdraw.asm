...
;;
;; $Header: /home/ktk/tmp/odin/2007/netlabs.cvs/odin32/src/opengl/glide/Voodoo1/glide/Attic/xdraw.asm,v 1.1 2000-02-18 10:43:16 sandervl Exp $
;; $Log: xdraw.asm,v $
;; Revision 1.1  2000-02-18 10:43:16  sandervl
...
EXTRN   GlideRoot:DWORD
EXTRN   grSpinFifo:PROC
        
; some useful floating load and store macros <ala gmt>
flds    EQU <fld  DWORD PTR>
fsubs   EQU <fsub DWORD PTR>
fmuls   EQU <fmul DWORD PTR>
...
fa      EQU     <eax>       ; vtx a from caller
fb      EQU     <ebx>       ; vtx b from caller
fc      EQU     <ecx>       ; vtx c from caller
                                ; edx is used as index, loading from *src
gc      EQU     <esi>       ; points to graphics context
dlp     EQU     <esi>       ; points to dataList structure
hw      EQU     <edi>       ; points to the hardware

tmpx    EQU     <edx>       ; temp X storage
i       EQU     <edx>       ; i = dlp->i
tmpy    EQU     <ebp>       ; temp Y storage
...
;--------------------------------------------------------------------------        
    mov     fa, [esp + _va$]    ; 1
     mov     fb, [esp + _vb$]
    mov     fc, [esp + _vc$]
     mov     tmpy, [GlideRoot + trisProcessed]    ; GlideRoot.stats.trisProcessed++;
; 36-3
vertex_y_load:
    mov     fa, [fa + Y]        ; 2
     mov     fb, [fb + Y]
...
IFDEF STATS
     inc     tmpy               ; GlideRoot.stats.trisProcessed++;
    mov [GlideRoot + trisProcessed], tmpy
ENDIF
     mov     tmpy, [GlideRoot + curTriSize] ; load GlideRoot.curTriSize
; 69-12
    ...
    push   edx
    push   tmpy
    call   grSpinFifo          ; fifoFree = grSpinFifo(gc->curTriSize);
    add    esp,08h
    mov    hw, eax              ; fifoFree = ...
    pop    edx
    pop    ecx
...
IF 1
  ...
  no_p6_2:
    ...
    mov     tmpy, [dlp + SIZEOF_dataList + dl_i] ; tmpy = dlp[1]->i ; preload next i
     mov    fa, [GlideRoot + trisDrawn];  GlideRoot.stats.trisDrawn++
    jmp     dotri
...
IFDEF STATS
     inc    fa                          ;  GlideRoot.stats.trisDrawn++
    mov    [GlideRoot + trisDrawn], fa
ENDIF
     mov    tmpx, zArea

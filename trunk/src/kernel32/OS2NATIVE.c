/* $Id: OS2NATIVE.c,v 1.1 1999-05-24 20:19:42 ktk Exp $ */
...
#endif
//******************************************************************************
extern ULONG _System timeGetTime();
  ...
  if(fdwAllocationType & MEM_COMMIT && lpvAddress != NULL) {
...
#if 0
        rc = DosSetMem(lpvAddress, cbSize, PAG_COMMIT | PAG_READ | PAG_WRITE);
        if(rc != 0)
                return(NULL);
#else
    ...
    if(rc == ERROR_ACCESS_DENIED && cbSize > 4096 ) {//knut: AND more than one page
        ...
        while(newbase >= (char *)lpvAddress) {  //knut: should check first page to!!
                ...
                if(rc == 0) {
#ifdef DEBUG1
                        dprintf(("Found new base %X, flag %d\n", newbase, flag));
                        dprintf(("Commit %d bytes extra\n", cbSize - ((int)newbase - (int)lpvAddress)));
#endif
...
#ifdef DEBUG
                                ULONG ressize = cbSize - ((int)newbase - (int)lpvAddress);
                                if(ressize & 0xFFF) {
                                        ressize = ((ressize >> 12) + 1)*4096;
                                }
                                else    ressize = (ressize >> 12)*4096;
                                reserve += ressize;
#endif

/* $Id: shlfolder.cpp,v 1.15 2000-08-18 02:01:23 phaller Exp $ */
/*
 * Shell Folder stuff
 *
...
extern struct ICOM_VTABLE(IUnknown) unkvt;
extern struct ICOM_VTABLE(IShellFolder2) sfvt;
extern struct ICOM_VTABLE(IPersistFolder2) psfvt;
extern struct ICOM_VTABLE(IDropTarget) dtvt;
extern struct ICOM_VTABLE(ISFHelper) shvt;
...
struct ICOM_VTABLE(IUnknown) unkvt =
{
   ...
   if(pidl && iface)                                    /* do we have a pidl? */
   {
     ...
     if (!_ILIsSpecialFolder(pidl))               /* only file system paths */
     {
       ...
       *(sf->sMyPath)=0x00;
       ...
       _ILSimpleGetText(pidl, sf->sMyPath + len, dwSize - len + 2);
     }
   ...
   LPENUMIDLIST* ppEnumIDList)
{
  ...
  *ppEnumIDList = NULL;
  ...
  {
   /* if This->sMyPath is NULL, we will start enumerating objects
      at MyComputer level */
...
static HRESULT WINAPI IShellFolder_fnGetAttributesOf(IShellFolder2 * iface,UINT cidl,LPCITEMIDLIST *apidl,DWORD *rgfInOut)
{
   ...
   {    //      return default
        *rgfInOut = SFGAO_CAPABILITYMASK|SFGAO_FILESYSTEM;
        return hr;
   }
...
ICOM_VTABLE(IShellFolder2) sfvt =
{
   ICOM_MSVTABLE_COMPAT_DummyRTTIVALUE
   IShellFolder_fnQueryInterface,
...
*/
extern struct ICOM_VTABLE(IShellFolder2) sfdvt;
...
ICOM_VTABLE(IShellFolder2) sfdvt =
{
   ICOM_MSVTABLE_COMPAT_DummyRTTIVALUE
   ISF_Desktop_fnQueryInterface,
   IShellFolder_fnAddRef,
...
extern struct ICOM_VTABLE(IShellFolder2) sfmcvt;

static shvheader MyComputerSFHeader [] =
{
 { IDS_SHV_COLUMN1, SHCOLSTATE_TYPE_STR | SHCOLSTATE_ONBYDEFAULT, LVCFMT_RIGHT, 15 },
 { IDS_SHV_COLUMN3, SHCOLSTATE_TYPE_STR | SHCOLSTATE_ONBYDEFAULT, LVCFMT_RIGHT, 10 },
 { IDS_SHV_COLUMN3DV, SHCOLSTATE_TYPE_STR | SHCOLSTATE_ONBYDEFAULT, LVCFMT_RIGHT, 10 },
 { IDS_SHV_COLUMN4DV, SHCOLSTATE_TYPE_STR | SHCOLSTATE_ONBYDEFAULT, LVCFMT_RIGHT, 10 },
};
     ...
     switch(iColumn)
     {
       ...
       case 2: /* total size */
         if (_ILIsDrive(pidl))
         {
           ...
           //CB: WINE uses ulBytes.s.LowPart
...
ICOM_VTABLE(IShellFolder2) sfmcvt =
{
   ICOM_MSVTABLE_COMPAT_DummyRTTIVALUE
   IShellFolder_fnQueryInterface,
   IShellFolder_fnAddRef,
   ...
   LPCITEMIDLIST                         pidl)
{
  ...
  {
    This->sMyPath = (CHAR*)SHAlloc(strlen(sTemp+1));
...
ICOM_VTABLE(IPersistFolder2) psfvt =
{
   ICOM_MSVTABLE_COMPAT_DummyRTTIVALUE

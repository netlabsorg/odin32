 ...
 *  init_handle_table
 *
 * Initializes the HANDLETBL structure pointed to by lpTable
 *
 * PARAMS
 *  lpTable [I] Pointer to the HANDLETBL structure, which is to be initalized.
 *
 * NOTES
 ...
 *  variable of type HANDLETBL for your handle table. However, in this
 ...
 */
static void release_all_handles(HANDLETBL *lpTable)
{
    unsigned int i;
 ...
 *  variable of type HANDLETBL and call init_handle_table on your own. 
 */
int alloc_handle_table(HANDLETBL **lplpTable)
{
    TRACE("(lplpTable=%p)\n", lplpTable);
        
    *lplpTable = HeapAlloc(GetProcessHeap(), 0, sizeof(HANDLETBL));
    if (*lplpTable) 
    {
 ...
 */
static int grow_handle_table(HANDLETBL *lpTable) 
{
    HANDLETBLENTRY *newEntries;
    unsigned int i, newIEntries;

    newIEntries = lpTable->iEntries + TABLE_SIZE_INCREMENT;

    newEntries = HeapAlloc(GetProcessHeap(), 0, sizeof(HANDLETBLENTRY)*newIEntries);
    if (!newEntries) 
        return 0;

    if (lpTable->paEntries)
    {
        memcpy(newEntries, lpTable->paEntries, sizeof(HANDLETBLENTRY)*lpTable->iEntries);
        HeapFree(GetProcessHeap(), 0, lpTable->paEntries);
    }
 ...
 */
static int alloc_handle(HANDLETBL *lpTable, OBJECTHDR *lpObject, HCRYPTKEY *lpHandle)
{
    int ret = 0;

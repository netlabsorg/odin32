...
static inline PWSTR CRYPT_GetProvKeyName(PCWSTR pProvName)
{
	...
	keyname = (PWSTR)CRYPT_Alloc((strlenW(KEYSTR) + strlenW(pProvName) +1)*sizeof(WCHAR));
	if (keyname)
	{
...
static inline PWSTR CRYPT_GetTypeKeyName(DWORD dwType, BOOL user)
{
	...
	keyname = (PWSTR)CRYPT_Alloc( ((user ? strlenW(USERSTR) : strlenW(MACHINESTR)) +1)*sizeof(WCHAR));
	if (keyname)
	{
...
static inline BOOL CRYPT_UnicodeToANSI(LPCWSTR wstr, LPSTR* str, int strsize)
{
	...
	if (strsize == -1)
		*str = (LPSTR)CRYPT_Alloc(count * sizeof(CHAR));
	else
...
static inline BOOL CRYPT_ANSIToUnicode(LPCSTR str, LPWSTR* wstr, int wstrsize)
{
	...
	if (wstrsize == -1)
		*wstr = (PWSTR)CRYPT_Alloc(wcount * sizeof(WCHAR));
	else

...
{
  dprintf(("MSVCRT_free %x ptr",ptr));
  HeapFree(GetProcessHeap(),0,ptr);
}

void* MSVCRT(memset)(void* ptr,int fill,MSVCRT(size_t) size)
{
  dprintf(("MSVCRT_memset %x(%d bytes) with %d",ptr,size,fill));
...
{
  dprintf(("MSVCRT_memcpy %x->%x (%d bytes)",ptr2,ptr,size));
...
void* MSVCRT_malloc(MSVCRT_size_t size)
{
  ...
  dprintf(("MSVCRT_malloc of %d size returned %x",size,ret));
  if (!ret)
    MSVCRT__set_errno(GetLastError());
  return ret;
}

			    ...
			    LPMDICREATESTRUCTA cs, BOOL unicode )
{
    ...
    /* fix window style */
#ifdef __WIN32OS2__
    if( !(GetWindowLongA(parent, GWL_STYLE) & MDIS_ALLCHILDSTYLES) )
#else
        ...
        case WM_NEXTMENU:
            {
                ...
                if (!IsIconic(hwnd) && ci->hwndActiveChild && !ci->hwndChildMaximized)
                {
                    ...
                    if( (wParam == VK_LEFT && GetMenu(hwnd) == next_menu->hmenuIn) ||
                        (wParam == VK_RIGHT && GetSubMenu(getSysMenu(hwnd), 0) == next_menu->hmenuIn) )
                    {
                        next_menu->hmenuNext = GetSubMenu(getSysMenu(ci->hwndActiveChild), 0);
    ...
    case WM_NEXTMENU:
        {
            ...
            {
#ifdef __WIN32OS2__
                next_menu->hmenuNext = GetSubMenu( getSysMenu(parent), 0 );
#else
...
#ifdef __WIN32OS2__
                RECT rect;
                GetWindowRectParent(list[i], &rect);

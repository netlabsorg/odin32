...
struct linger xx;
	...
	xx.l_linger = (int)yy->l_linger;
        ...
        return setsockopt(s,level,optname,(const char *)&xx,optlen);
    } 
...
struct hostent *xx;
    ...
    xx = gethostbyaddr(addr,len,type);
...
AsyncStatus *as;
    ...
    do {
        rc = select(socks,r,0,0,0); // ioctl may be better for this.
        if(rc > 0) {

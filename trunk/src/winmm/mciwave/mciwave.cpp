...
{
    struct SCA*	sca = (SCA*)HeapAlloc(GetProcessHeap(), 0, sizeof(struct SCA));
...
static	DWORD WAVE_mciReadFmt(WINE_MCIWAVE* wmw, MMCKINFO* pckMainRIFF)
{
    ...
    wmw->lpWaveFormat = (WAVEFORMATEX*)HeapAlloc(GetProcessHeap(), 0, mmckInfo.cksize);
					      ...
					      DWORD dwParam1, DWORD dwParam2)
{
	...
	break;
    case WOM_DONE:
	InterlockedIncrement((LPLONG)&wmw->dwEventCount);
...
static void WAVE_mciPlayWaitDone(WINE_MCIWAVE* wmw)
{
	...
	if (InterlockedDecrement((LPLONG)&wmw->dwEventCount) >= 0) {
	    break;
	}
	InterlockedIncrement((LPLONG)&wmw->dwEventCount);
...
((((v) + (wmw)->lpWaveFormat->nBlockAlign - 1) / (wmw)->lpWaveFormat->nBlockAlign) * (wmw)->lpWaveFormat->nBlockAlign)
    ...
    waveHdr = (wavehdr_tag*)HeapAlloc(GetProcessHeap(), 0, 2 * sizeof(WAVEHDR) + 2 * bufsize);

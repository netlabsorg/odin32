...
void DPLAYX_PrivHeapFree( LPVOID addr )
{
  ...
  lpAddrStart = (LPVOID)((DWORD)addr - sizeof(DWORD)); /* Find block header */
...
LPSTR DPLAYX_strdupA( DWORD flags, LPCSTR str )
{
  LPSTR p = (LPSTR)DPLAYX_PrivHeapAlloc( flags, strlen(str) + 1 );
  if(p) {
    strcpy( p, str );
  }
  return p;
...
LPWSTR DPLAYX_strdupW( DWORD flags, LPCWSTR str )
{
  ...
  LPWSTR p = (LPWSTR)DPLAYX_PrivHeapAlloc( flags, len * sizeof(WCHAR) );
  if(p) {
    strcpyW( p, str );
  }
  return p;
...
BOOL DPLAYX_ConstructData(void)
{
  ...
  lpMemArea = (DPLAYX_MEM_SLICE*)((BYTE*)lpSharedStaticData + dwStaticSharedSize);
  ...
  LPDPLCONNECTION lpConn )
{
  ...
  lpDplData->lpConn = (LPDPLCONNECTION) DPLAYX_PrivHeapAlloc( HEAP_ZERO_MEMORY,

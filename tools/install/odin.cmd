...
rc = lineout(sInstFile, '<REXX NAME="RequiresLibc">');
rc = lineout(sInstFile, '  call rxFuncAdd 'SysLoadFuncs','REXXUTIL','SysLoadFuncs'');
rc = lineout(sInstFile, '  call SysLoadFuncs');
rc = lineout(sInstFile, '  dname = "libc063.dll"');
rc = lineout(sInstFile, '  requires = ''REQUIRES="netlabs.org\kLIBC\LIBC 0.6 Runtime\0\6\3"''');
rc = lineout(sInstFile, '  cf = SysBootDrive() || "\config.sys"');
rc = lineout(sInstFile, '  lp = "LIBPATH="');
rc = lineout(sInstFile, '  call SysFileSearch lp, cf, "lp."');
rc = lineout(sInstFile, '  lpath = ""');
rc = lineout(sInstFile, '  do i=1 to lp.0');
rc = lineout(sInstFile, '    if pos(lp, lp.i) = 1 then do');
rc = lineout(sInstFile, '      lpath = substr(lp.i, 9)');
rc = lineout(sInstFile, '    end');
rc = lineout(sInstFile, '  end');
rc = lineout(sInstFile, '  cr = d2c(10)');
rc = lineout(sInstFile, '  do while lpath \= ""');
rc = lineout(sInstFile, '    parse var lpath ldir ";" lpath');
rc = lineout(sInstFile, '    tfile = ldir || "\" || dname');
rc = lineout(sInstFile, '    if stream(tfile, "c", "query exists") \= "" then do');
rc = lineout(sInstFile, '      return ""');
rc = lineout(sInstFile, '    end');
rc = lineout(sInstFile, '  end');
rc = lineout(sInstFile, '  /* file not found, return REQUIRES clause */');
rc = lineout(sInstFile, '  return requires');
rc = lineout(sInstFile, '</REXX>');
...
rc = lineout(sInstFile, '     =("RequiresLibc")');
...
rc = lineout(sInstFile, '     Dmitry A. Kuminov');

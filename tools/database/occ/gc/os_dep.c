...
{
    void * pvResult;
    //kso: use high memory if possible!
    APIRET  rc;
    static ULONG flAllocMem = ~0UL;
    #ifndef QSV_VIRTUALADDRESSLIMIT
    #define QSV_VIRTUALADDRESSLIMIT 30
    #endif
    #ifndef OBJ_ANY
    #define OBJ_ANY 0x00000400U
    #endif
    if (flAllocMem == ~0UL)
    {
        ULONG   ulSysInfo = 0;
        flAllocMem = PAG_READ | PAG_WRITE | PAG_COMMIT;
        rc = DosQuerySysInfo(QSV_VIRTUALADDRESSLIMIT, QSV_VIRTUALADDRESSLIMIT, &ulSysInfo, sizeof(ulSysInfo));
        flAllocMem |= !rc && ulSysInfo > 512 ? OBJ_ANY : 0;
    }

    rc = DosAllocMem(&pvResult, bytes, flAllocMem);
    if (rc)
    {
        fprintf(stderr, "Shit! Shit! Shit!!! We're out of memory, DosAllocMem rc=%d\n", rc);
	    return(NULL);
    }

    #ifdef DEBUG
    if (bytes & 0xfff)                  /* page size for now. */
        fprintf(stderr, "os2_alloc - debug: missaligned big memory block allocated. 0x%x bytes\n", bytes);
    #endif
